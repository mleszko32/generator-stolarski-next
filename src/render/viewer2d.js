...
export function generateSidePanelSVG(height, depth, mountingData) {
  ...
  if (shelfHoles && shelfHoles.length > 0) {
    shelfHoles.forEach(hole => {
      ...
      const cxL = bokLX + hole.x;
      const cxR = bokRX + depth - hole.x;
      
      svg += `<g class="${layerClass}">`;
      svg += `<circle cx="${cxL}" cy="${svgY}" r="${radius}" fill="${color}" />`;
      svg += `<circle cx="${cxR}" cy="${svgY}" r="${radius}" fill="${color}" />`;
  ...
  // --- WEWNĘTRZNE WYMIAROWANIE MIĘDZY PÓŁKAMI RUCHOMYMI ---
  ...
  // --- LEWY BOK: Poziome wymiary od przedniej krawędzi ---
  if (mountingData) {
