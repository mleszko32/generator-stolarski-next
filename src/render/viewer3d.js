...
// Funkcja głęboko czyszcząca pamięć (Mesh, Geometry, Material)
...
export function update3D() {
  ...
  // Globalne materiały dla optymalizacji wydajności
  ...
  // PĘTLA PO WSZYSTKICH MODUŁACH W PROJEKCIE
  state.project.modules.forEach(mod => {
    
    // Tworzymy lokalną grupę dla danej szafki i przesuwamy ją na pozycję z projektu
    ...
    // Funkcja budująca obiekty DODAJĄCA JE DO LOKALNEJ GRUPY MODUŁU (moduleGroup)
    const createBoard = (w, h, d, x, y, z, material, lineMaterial) => {
      const geo = new THREE.BoxGeometry(w, h, d);
      const mesh = new THREE.Mesh(geo, material);
      mesh.position.set(x + w/2, y + h/2, z + d/2);
      moduleGroup.add(mesh); // Tu następuje zmiana - dodajemy do szafki, nie do całej sceny
    ...
    // --- RYSOWANIE PÓŁEK, PRZEGRÓD I FRONTÓW ---
    if (mod.elements) {
  ...
  // Automatyczne ustawienie kamery na całą kuchnię
